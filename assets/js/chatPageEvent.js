...
var initPage = myApp.onPageAfterAnimation('chat', function(page) {
  ...
  io.socket.post('/room/' + postId + '/users', function(body, sailsResponseObject) {
    console.log('Sails responded with: ', body);
    console.log('and with status code: ', sailsResponseObject.statusCode);
    console.log("members=>", body.room.members[0]);
  });
...
function initMsg(roomInfo) {
  // say something
  var users = "";
  for (var i = 0; i < roomInfo.room.members.length; i++) {
    users += roomInfo.room.members[i].username + "/";
    console.log("roomInfo.room.members[i]=>", roomInfo.room.members[i]);
  }
