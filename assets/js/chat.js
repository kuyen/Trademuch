...
sails.sockets.on('connect', function socketConnected() {
    ...
    sails.sockets.on('room', function messageReceived(message) {
    ...
    // to subscribed sockets.
    sails.sockets.on('user', function messageReceived(message) {
    ...
    sails.sockets.on('disconnect', function() {
      // Hide the main UI
      $('#main').hide();
      $('#disconnect').show();
    });
